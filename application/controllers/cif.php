...
class Cif extends GMN_Controller {
	...
	public function datatable_target_cabang()
	{
		/* Array of database columns which should be read and sent back to DataTables. Use a space where
		 * you want to insert a non-database field (for example a counter or static image)
		 */
		...
		$branch_code = @$_GET['branch_code'];
		$aColumns = array( '','tahun','item_target','');
		...
		$rResult 			= $this->model_cif->datatable_target_cabang($sWhere,$sOrder,$sLimit); // query get data to view
		$rResultFilterTotal = $this->model_cif->datatable_target_cabang($sWhere); // get number of filtered data
		$iFilteredTotal 	= count($rResultFilterTotal); 
		$rResultTotal 		= $this->model_cif->datatable_target_cabang(); // get number of all data
		$iTotal 			= count($rResultTotal);	
		...
		{
			$row = array();
			$row[] = '<input type="checkbox" value="'.$aRow['target_id'].'" id="checkbox" class="checkboxes" >';
			$row[] = $aRow['tahun'];
			$row[] = $aRow['item_target']; 
			$row[] = $aRow['t1']; 
			$row[] = $aRow['t2'];
			$row[] = $aRow['t3'];
			$row[] = $aRow['t4'];
			...
			$row[] = '<a href="javascript:;" target_id="'.$aRow['target_id'].'" id="link-edit">Edit</a>';
	...
	public function add_target_cabang()
	{
		$branch_code 		= $this->input->post('add_branch_code');
		...
		$this->db->trans_begin();
		$this->model_cif->add_target_cabang($data);
		if($this->db->trans_status()===true){
			$this->db->trans_commit();
			$return = array('success'=>true);
		}else{
	...
	public function get_data_target_by_target_id()
	{
		$target_id = $this->input->post('target_id');
		$data = $this->model_cif->get_data_target_by_target_id($target_id);

		echo json_encode($data);
	}

	public function edit_target_cabang()
	{
		$target_id 		= $this->input->post('target_id');
		$t1 			= $this->input->post('t1');
		$t2 			= $this->input->post('t2');
		...
		$param = array('target_id'=>$target_id);
		$data = array(
				't1'			=> $t1, 
				't2'			=> $t2, 
				't3'			=> $t3, 
		...
		$this->db->trans_begin();
		$this->model_cif->edit_target_cabang($data,$param);
		if($this->db->trans_status()===true){
			$this->db->trans_commit();
			$return = array('success'=>true);
		}else{
	...
	public function search_target_item()
	{
		$branch_code 	= $this->input->post('branch_code');
		$data 			= $this->model_cif->search_target_item($branch_code);
