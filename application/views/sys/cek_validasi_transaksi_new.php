    ...
    <table id="filter-form">
       <tr>
          ...
          <td>
             ...
             <input type="text" name="branch" id="branch" class="m-wrap" value="<?php echo $this->session->userdata('branch_name'); ?>" readonly style="background:#EEE"> 
       ...
       <tr>
          ...
          <td>
             <input type="text" name="from_date" id="from_date" class="m-wrap date-picker mask_date" placeholder="dd/mm/yyyy" value="<?php echo date('d/m/Y') ?>">
             <span style="line-height:40px;height:40px;">s.d &nbsp;</span>
             <input type="text" name="thru_date" id="thru_date" class="m-wrap date-picker mask_date" placeholder="dd/mm/yyyy" value="<?php echo date('d/m/Y') ?>">
          </td>
       </tr>
       <tr>
          <td style="padding-bottom:10px;">&nbsp;</td>
          <td>
            <button id="filter_financing" class="btn green"><i class="icon-search"></i> Cek Pembiayaan</button>
            <button id="filter_saving" class="btn green"><i class="icon-search"></i> Cek Tabungan</button>
          </td>
       </tr>
    </table>
    <hr size="1">
    ...
    <table width="70%" border="1" id="cek-outstanding" cellpadding="5">
      <thead>
        <tr>
          ...
          <th style="background:#ccc;" width="20%">Action</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td colspan="5" style="padding:5px;font-size:11px;">Data is Empty...</td>
        </tr>
      </tbody>
    </table>
    <br>
    <table width="70%" border="1" id="cek-saving" cellpadding="5">
      <thead>
        <tr>
          <th style="background:#ccc;" width="40%">&nbsp;</th>
          <th style="background:#ccc;" width="20%">Rinci</th>
...
$(function(){
   ...
   $("#filter_financing").click(function(e){
    e.preventDefault();
    ...
    $.ajax({
      ...
      url: site_url+'sys/load_data_cek_financing_new',
      success:function(response){
        $("#cek-outstanding tbody").html(response);
      }
    });
    /*
    $.ajax({
      ...
      url: site_url+"sys/load_data_cek_validasi_transaksi_new",
      // async:false,
      success:function(response){
        $("#cek-validasi tbody").html(response);
      }
    });
    */
   })
   $("#filter_saving").click(function(e){
    e.preventDefault();
    var branch_code = $("#branch_code").val();
    var from_date = $("#from_date").val();
    var thru_date = $("#thru_date").val();
    ...
    $.ajax({
      ...
      url: site_url+'sys/load_data_cek_saving_new',
      success:function(response){
        $("#cek-saving tbody").html(response);
      }
    });
  });
