...
<div class="row-fluid">
   <div class="span12">
      ...
      <h3 class="form-section">
        KOREKSI PEMBULATAN WAJIB KELOMPOK <small></small>
      </h3>
      <!-- END PAGE TITLE-->
   </div>
</div>
...
<div class="portlet box blue" id="wrapper-table">
   <div class="portlet-title">
      <div class="caption"><i class="icon-globe"></i>List Koreksi Tabungan Sukarela</div>
      <div class="tools">
   ...
   <div class="portlet-body">
      <div class="clearfix">
         ...
         <form action="<?php echo site_url('koreksi_pembulatan/proses_koreksi'); ?>" method="post">
            ...
            <table id="filter-form">
               ...
               <tr>
                  ...
                  <td>
                     ...
                     <button class="green btn" id="koreksi" type="submit">Koreksi</button>
                  </td>
               </tr>
            </table>
         </form>
...
var grid = function(){
	jQuery("#list485").jqGrid({
		url: site_url+'koreksi_pembulatan/jqgrid_list_koreksi_pembulatan',
		//data: mydata,
		datatype: 'json',
		height: 'auto',
		autowidth: true,
		...
		colNames:['CIF','Nama','Tabungan Wajib','Tabungan Kelompok','Majelis'],
		colModel:[
...
/* BEGIN DIALOG ACTION BRANCH */
   ...
   $("#browse_rembug").click(function(){
      $.ajax({
         ...
         success: function(response){
            html = '<option value="00000" cm_id="" cm_name="SEMUA MAJELIS">00000 - SEMUA MAJELIS</option>';
            for ( i = 0 ; i < response.length ; i++ )
            {
   ...
   $("#keyword","#dialog_rembug").keyup(function(e){
      ...
      if(e.which==13)
      {
         $.ajax({
            ...
            success: function(response){
              html = '';
              html += '<option value="00000" cm_id="00000" cm_name="SEMUA MAJELIS">00000 - SEMUA MAJELIS</option>';
              for ( i = 0 ; i < response.length ; i++ )
              {
                 html += '<option value="'+response[i].cm_code+'" cm_id="'+response[i].branch_id+'" cm_name="'+response[i].cm_name+'">'+response[i].cm_code+' - '+response[i].cm_name+'</option>';
              }
