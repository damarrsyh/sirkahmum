...
<div class="portlet box blue">
  <div class="portlet-title">
     <div class="caption"><i class="icon-reorder"></i>LIST ANGGOTA MASUK</div>
     <div class="tools">
     </div>
  </div>
  ...
  <div class="portlet-body form">
     ...
     <form action="#" method="post" enctype="multipart/form-data" id="form_add" class="form-horizontal">
        ...
        <div class="form-actions">
           ...
           <button type="submit" id="export_csv" class="btn green">CSV</button>
        </div>
     </form>
     <!-- END FORM-->
      <p><hr></p>
...
var showin = function(){
  ...
  jQuery("#list485").jqGrid({
    url: site_url+'laporan/jqgrid_list_anggota_masuk',
    //data: mydata,
    datatype: 'json',
    height: 'auto',
    autowidth: true,
    ...
    colNames:['ID','Nama','Majelis','Tanggal Gabung','Jenis Kelamin','Ibu Kandung','Tempat Lahir','Tanggal Lahir','Usia','Alamat'],
    colModel:[
      {name:'cif_no',index:'cif_no'},
      {name:'nama',index:'nama'},
      {name:'cm_code',index:'cm_name'},
      {name:'tgl_gabung',index:'tgl_gabung'},
      {name:'jenis_kelamin',index:'jenis_kelamin'},
      {name:'ibu_kandung',index:'ibu_kandung'},
      {name:'tmp_lahir',index:'tmp_lahir'},
      {name:'tgl_lahir',index:'tgl_lahir'},
      {name:'usia',index:'usia'},
      {name:'alamat',index:'alamat'},
    ],
    shrinkToFit: true,
    pager: "#plist485",
    viewrecords: true,
...
$("#export_excel").click(function(e){
  ...
  window.open(site_url+"laporan_to_excel/export_list_anggota_masuk/"+branch_code+"/"+cm_code+"/"+tanggal+"/"+tanggal2);
});
...
$("#export_csv").click(function(e){
  ...
  tanggal = $("#tanggal").val().replace(/\//g,'');
  tanggal2 = $("#tanggal2").val().replace(/\//g,'');
  window.open(site_url+"laporan_to_csv/export_list_anggota_masuk/"+branch_code+"/"+cm_code+"/"+tanggal+"/"+tanggal2);
});
...
$("#export_pdf").click(function(e){
  ...
  tanggal = $("#tanggal").val().replace(/\//g,'');
  tanggal2 = $("#tanggal2").val().replace(/\//g,'');
  window.open(site_url+"laporan_to_pdf/export_list_anggota_masuk/"+branch_code+"/"+cm_code+"/"+tanggal+"/"+tanggal2);
});
