...
<div class="portlet box blue">
  <div class="portlet-title">
     <div class="caption"><i class="icon-reorder"></i>LIST MUTASI ANGGOTA </div>
     <div class="tools">
     </div>
  </div>
  ...
  <div class="portlet-body form">
     ...
     <form action="#" method="post" enctype="multipart/form-data" id="form_add" class="form-horizontal">
        ...
        <div class="control-group">
           ...
           <div class="controls">
            <select class="m-wrap medium" name="alasan" id="alasan">
        ...
        <div class="form-actions">
           ...
           <button type="submit" id="export_pdf" class="btn green" >View PDF</button>
           <button type="submit" id="export_excel" class="btn green">View Excel</button>
           <button type="submit" id="export_csv" class="btn green">View CSV</button>
        </div>
     </form>
     <!-- END FORM-->
      <p><hr></p>
...
var showin = function(){
  ...
  jQuery("#list485").jqGrid({
    url: site_url+'laporan/jqgrid_list_anggota_mutasi',
    //data: mydata,
    datatype: 'json',
    height: 'auto',
    autowidth: true,
    postData: {
      ...
      alasan : function(){return $("#alasan").val()},
      cm_code : function(){return $("#cm_code").val()},
    },
    rowNum: 30,
    rowList: [50,100,150,200],
    colNames:['Tanggal Mutasi','ID','Nama','Majlis Awal','Majlis Baru','Alasan Mutasi','Keterangan'],
    colModel:[
      {name:'tanggal_mutasi',index:'tanggal_mutasi'},
      {name:'cif_no',index:'cif_no'},
      {name:'nama',index:'nama'},
      {name:'rembug_lama',index:'rembug_lama'},
      {name:'rembug_baru',index:'rembug_baru'},       
      {name:'alasan_mutasi',index:'alasan_mutasi'},
      {name:'alasan',index:'alasan'},
    ],
    shrinkToFit: true,
    pager: "#plist485",
    viewrecords: true,
...
$("#export_excel").click(function(e){
  ...
  alasan = $("#alasan").val();
  window.open(site_url+"laporan_to_excel/export_list_anggota_mutasi/"+branch_code+"/"+cm_code+"/"+tanggal+"/"+tanggal2+"/"+alasan+"/preview");
});
...
$("#export_csv").click(function(e){
  ...
  tanggal2 = $("#tanggal2").val().replace(/\//g,'');
  alasan = $("#alasan").val();
  window.open(site_url+"laporan_to_csv/export_list_anggota_mutasi/"+branch_code+"/"+cm_code+"/"+tanggal+"/"+tanggal2+"/"+alasan+"/preview");
});
...
$("#export_pdf").click(function(e){
  ...
  tanggal = $("#tanggal").val().replace(/\//g,'');
  tanggal2 = $("#tanggal2").val().replace(/\//g,'');
  window.open(site_url+"laporan_to_pdf/export_list_anggota_mutasi/"+branch_code+"/"+cm_code+"/"+tanggal+"/"+tanggal2);
});
